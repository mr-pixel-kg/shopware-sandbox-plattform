  ...
  methods: {
    ...
    async deleteSandbox() {
      ...
      if (resp.success === true) {
        ...
        this.generalStore.removeSandbox2(this.sandbox.id);
        console.log("called delete sandbox in active sandbox card");
        this.$emit("delete-sandbox", this.sandbox.id);
      } else {
        this.$toast.add({
          severity: "error",
...
<template>
  <Card class="w-full max-w-lg shadow-lg border border-gray-200" style="overflow: hidden;">
    <template #title>Shopware Sandbox</template>
    <template #header>
      <ProgressBar :value="100 - (sandbox.getRemainingTime().split('m')[0] * 100 / 60)" :show-value="false" style="height: 5px;"></ProgressBar>
    </template>
    <template #subtitle>
    ...
    <template #content>
      <div class="space-y-2">
        <div class="flex justify-between items-center">
          ...
          <Tag :class="statusColor" class="px-2 py-1 text-white">{{ sandbox.status }}</Tag>
        </div>
        <div class="flex justify-between items-center">
    ...
    <template #footer>
      <div class="flex gap-2 mt-3">
        <Button icon="pi pi-trash" severity="danger" class="w-1/4" @click="deleteSandbox" />
        <a class="w-1/2" :href="sandbox.getStorefrontUrl()" target="_blank">
          <Button label="Storefront" severity="primary" class="w-full" @click="openUrl(sandbox.getStorefrontUrl())" />
        </a>

        <a class="w-1/2" :href="sandbox.getAdminUrl()" target="_blank">
          <Button label="Admin" severity="secondary" class="w-full" @click="openUrl(sandbox.getAdminUrl())" />
        </a>
      </div>
    </template>
  </Card>
